...
export const FirstScreenGuide = ({ screenId }: { screenId?: string }) => {
  ...
  return <section aria-label="Screen setup help" className="rounded-xl border border-surface-highlight bg-surface p-5 sm:p-6 mb-8">
    ...
    {state && <ol className="grid md:grid-cols-3 gap-6 mt-6">
      ...
      <li><h3 className="font-semibold">2. Connect your screen</h3><p className="text-text-secondary my-3">{screen ? `Open setup for ${screen.name}, choose its content, then follow the connection instructions on the device attached to your display.` : 'Add a screen, choose its content, then follow the connection instructions.'}</p><Link className="ui-button ui-button-secondary" to={screenLink}>{screen ? 'Open screen setup' : 'Add a screen'}</Link>{screen && <a className="ui-button ui-button-secondary mt-2" href={`/player/${screen.id}`} target="_blank" rel="noreferrer">Preview in a new tab</a>}<p role="status" className="mt-3">{connection === 'connected' ? 'Screen connected. Check that your menu is visible on the display.' : connection === 'disconnected' ? 'No recent connection. Check that the player is open and connected to the internet.' : 'Waiting for a screen connection.'}</p><p className="text-sm text-text-secondary mt-2">A connection confirms that the screen device is responding. It does not confirm that the TV is on or the menu is visible. A preview tab is not a connected restaurant display.</p></li>
