...
export const SubscriptionManager = () => {
  ...
  return <section className="max-w-7xl mx-auto p-4 sm:p-8" aria-busy={busy}>
    ...
    <PlansPanel initialScreens={Math.max(1, organization?.screenCount || 1)} initialSeats={Math.max(1, organization?.members?.length || 1)} selectedPlan={selected || organization?.plan} currentPlan={organization?.plan} busy={busy} onChoose={(name, screens, seats) => void choose(name, screens, seats)} />
  </section>;
};
