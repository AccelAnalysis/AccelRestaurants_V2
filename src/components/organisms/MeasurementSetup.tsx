  ...
  return <div className="space-y-8">
    <section className="rounded-xl border border-surface-highlight bg-surface p-5 sm:p-6">
      ...
      <form onSubmit={create} className="grid sm:grid-cols-2 gap-4">
        ...
        <label className="space-y-2"><span>Experience</span><select className={inputClass} value={template} onChange={event => setTemplate(event.target.value as Template)}>
    ...
    <section className="rounded-xl border border-surface-highlight bg-surface p-5 sm:p-6">
      ...
      <form onSubmit={event => { event.preventDefault(); void run(() => MeasurementService.bind(orgId, campaignId, slideId, tileId), 'Campaign attached. Live players will receive the new slide revision.'); }} className="grid sm:grid-cols-3 gap-4">
        <label className="space-y-2"><span>Campaign</span><select required className={inputClass} value={campaignId} onChange={event => setCampaignId(event.target.value)}><option value="">Choose campaign</option>{campaigns.filter(c => c.status === 'active').map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
        <label className="space-y-2"><span>Slide</span><select required className={inputClass} value={slideId} onChange={event => { setSlideId(event.target.value); setTileId(''); }}><option value="">Choose slide</option>{slides.map(slide => <option key={slide.id} value={slide.id}>{slide.name}</option>)}</select></label>
        <label className="space-y-2"><span>QR tile</span><select required className={inputClass} value={tileId} onChange={event => setTileId(event.target.value)}><option value="">Choose QR</option>{availableTiles.map(tile => <option key={tile.id} value={tile.id}>{tile.name || tile.id}</option>)}</select></label>
