          ...
          <div className="max-w-2xl mx-auto space-y-8">
            ...
            <section className="space-y-4">
              ...
              {(['schedule','quietHours'] as const).map(key => {
                ...
                return <div key={key} className="p-3 border border-surface-highlight rounded space-y-2"><label className="flex items-center gap-2 text-sm text-text"><input type="checkbox" checked={schedule.enabled} onChange={e => setAudioConfig(prev => ({ ...prev, [key]: { ...schedule, enabled: e.target.checked } }))} /> {key === 'schedule' ? 'Screen audio schedule' : 'Quiet hours (scheduler override)'}</label><div className="grid grid-cols-2 gap-2"><input type="time" aria-label={`${key === 'schedule' ? 'Screen audio schedule' : 'Quiet hours'} start time`} value={schedule.startTime} onChange={e => setAudioConfig(prev => ({ ...prev, [key]: { ...schedule, startTime: e.target.value } }))} className="bg-background border border-surface-highlight rounded p-2 text-text" /><input type="time" aria-label={`${key === 'schedule' ? 'Screen audio schedule' : 'Quiet hours'} end time`} value={schedule.endTime || '23:59'} onChange={e => setAudioConfig(prev => ({ ...prev, [key]: { ...schedule, endTime: e.target.value } }))} className="bg-background border border-surface-highlight rounded p-2 text-text" /></div><input value={schedule.daysOfWeek.join(',')} onChange={e => setAudioConfig(prev => ({ ...prev, [key]: { ...schedule, daysOfWeek: e.target.value.split(',').map(Number).filter(day => day >= 0 && day <= 6) } }))} className="w-full bg-background border border-surface-highlight rounded p-2 text-text text-sm" aria-label={`${key} days of week`} /><p className="text-[10px] text-text-muted">Days use 0–6 = Sunday–Saturday. Overnight windows are supported.</p></div>;
              })}
