...
export const AtmospherePresetPanel = ({ config, plan, onChange, previewOnly = false }: Props) => {
  ...
  return <section aria-label="Atmosphere presets" className="space-y-4">
    ...
    {!available && <p className="text-sm text-text-secondary">Static presentation is included. Motion presets are {previewOnly ? 'preview-only on your plan' : 'included with Growth and above'}. <Link to="/admin/subscription" className="underline text-primary">Compare plans</Link></p>}
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
      {ATMOSPHERE_PRESETS.map(p => <button type="button" key={p.id} aria-pressed={selectedId === p.id}
        ...
        <span className="block font-semibold text-sm">{p.name}{p.id !== 'clear' && !available ? ' · Growth' : ''}</span><span className="block text-xs text-text-secondary mt-1">{p.description}</span>
      </button>)}
    </div>
    ...
    <p className="text-xs text-text-secondary">Reduced-motion preferences always win. Unsupported graphics fall back to a readable static board. Eco uses a smaller rendering budget.</p>
