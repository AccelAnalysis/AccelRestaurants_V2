// One-shot exact-source integration; runner commits source only and removes this helper.
import { readFileSync, writeFileSync } from 'node:fs';
const read = path => readFileSync(path, 'utf8');
const write = (path, value) => writeFileSync(path, value);
function patch(path, before, after) {
  const source = read(path);
  if (!source.includes(before)) throw new Error(`Missing source anchor in ${path}: ${before.slice(0, 90)}`);
...
// Unobservable off-site actions are not treated as non-conversions in the first-party engagement denominator.
patch(engine, "this.event(p, 'scan', { scans: 1 }, this.now(), { cohortScans: 1 })", "this.event(p, 'scan', { scans: 1 }, this.now(), p.kind === 'external' ? {} : { cohortScans: 1 })");
patch('src/components/organisms/MeasurementDashboard.tsx', 'of scan cohorts in this date range', 'of first-party offer/survey scan cohorts');
console.log('Source fixes applied. Production Firebase and workflow permissions were not changed.');
